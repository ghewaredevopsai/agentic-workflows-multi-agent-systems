...
The offline verify.py proves the object-level self-checks hold -- the splitter, the
Chroma collection, the bound tools, the compiled graph, the parser. Only this proves
that the tool choice, the cited answer, the model-as-judge and the refusal clause
actually work against the served model.

    python3 verify_live.py                 # every solution
    python3 verify_live.py lab-6-03        # just the ones whose name matches

Run it ON THE CLUSTER (or anywhere the LAB_LLM_* variables point at a live gateway).
It makes real model calls and costs real tokens -- a full Module 6 pass is about a cent.
"""
