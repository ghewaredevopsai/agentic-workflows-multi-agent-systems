...
def count_blanks(nb):
    ...
    A plain substring count cannot tell an unfilled blank from a guard that legitimately
    mentions the sentinel -- labs 6.2 and 6.4 compare a docstring or a field description
    against the literal "BLANK", and those read as unfilled blanks in the solution.
    Tokenising counts only real identifier uses, so it still catches every genuine one.
    """
    total = 0
    for c in nb["cells"]:
        if c["cell_type"] != "code":
