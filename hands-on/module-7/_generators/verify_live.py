...
This is the verifier that matters now that Module 7 is framework-forward: the offline
verify.py proves the object-level self-checks hold, but only this proves that the
create_agent runs, the callback tracer, the typed verdicts and the structured judge
...
The LANGFUSE_* names are scrubbed on purpose: no graded cell uses Langfuse, every Langfuse
cell degrades to a printed message, and verification must never depend on a tracing
