...
The ground-truth rule is stated here and nowhere else. It is the whole of the capstone's
...
What matters is the SIZE. Lab 7.1 measures what a small eval set can establish, which is
close to nothing -- at ten cases a single run ranges from 50% to 100%, and a 35-point
improvement is not detectable. A capstone gate built on seven cases would contradict the
module that precedes it. So the same rule is applied to 45 cases, and the seven cases
written by hand are the first seven.
"""
...
# The seven written by hand. Everything after them is generated from the same rule.
CANONICAL = {
...
def expected(record) -> str:
    """The correct answer for one payment, independent of any agent. The rule above."""
    if record is None:
        return "unknown"
...
def write_eval_set(cases: list) -> str:
    ...
    doc = {
        "_comment": ("GENERATED by _generators/gen_capstone.py -- do not edit by hand. "
                     "The ground-truth rule is stated in the generator; the number of "
                     "cases follows Lab 7.1, which showed what seven cases can establish."),
...
Four outcomes: `no action`, `proceed`, `hold for a human`, `unknown`. The rule is stated once, in
`domain.py`. What matters is the size: Lab 7.1 measured what a seven-case eval set can
