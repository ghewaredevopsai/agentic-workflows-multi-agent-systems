...
# and the solution are the same file on purpose. Lab 5.1 is one because every cell in it is
# a "Run it for real" cell -- a score computed from model output is a score a flaky endpoint
# can move, which is the whole reason the graded-cell rule exists. For those the rules invert:
# zero blanks on both sides, the two files byte-identical, and no score line expected. Every
# live cell is guarded on llm_ready(), which is false here, so the notebook must run clean
# and print its "Model not configured" message instead.
...
for fn in sorted(f for f in os.listdir(SOLDIR) if f.endswith(".ipynb")):
    ...
    if fn in WALKTHROUGH:
        # no self-checks by design; it passes if it executed cleanly and skipped its live
        # steps rather than reaching the gateway
        skipped = out.count("Model not configured")
        ok = n_fail == 0 and n_todo == 0 and n_guard == 0
        print(f"[{'OK    ' if ok else 'BROKEN'}] {fn:44} walkthrough: ran clean, "
              f"{skipped} live cell(s) self-skipped with no model")
    else:
...
def count_blanks(nb):
    ...
    legitimately mentions the sentinel -- Module 1's lab 1.3 needs to compare a docstring
    against the literal "BLANK", and that read as four unfilled blanks in the solution.
