...
This is the verifier that matters since Module 5 became framework-forward: the offline
verify.py proves the object-level self-checks hold, but only this proves that the
compiled graphs run, the reducers merge, the interrupts fire and the checkpointer
actually work against the served model.

    python3 verify_live.py                 # every solution
    python3 verify_live.py lab-5-03        # just the ones whose name matches

Run it ON THE CLUSTER (or anywhere the LAB_LLM_* variables point at a live gateway).
It makes real model calls and costs real tokens -- a full Module 5 pass is about a cent.
"""
