...
def count_blanks(nb):
    ...
    legitimately mentions the sentinel -- Module 8 has several (a Field description, a
    refusal clause), and each of them read as an unfilled blank in the SOLUTION and flipped
    the lab to [BROKEN]. Tokenising counts only real identifier uses, so it still catches
    every genuine leftover blank.
    """
    total = 0
    for c in nb["cells"]:
        if c["cell_type"] != "code":
