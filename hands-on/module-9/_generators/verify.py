...
for v in ("LAB_LLM_BASE_URL", "LAB_LLM_MODEL",
          ...
          # Module 9 only: a lab cell runs `kubectl apply --dry-run=server` against
          # whatever APP_NAMESPACE names. It is non-mutating, but a verifier that talks
          # to an API server is not offline and not deterministic, so it is scrubbed too.
...
def count_blanks(nb):
    ...
    legitimately mentions the sentinel -- in Module 1 that read as four unfilled blanks in a
    solution and flipped the lab to [BROKEN]. Tokenising counts only real identifier uses,
    so it still catches every genuine leftover blank.
    """
    total = 0
    for c in nb["cells"]:
        if c["cell_type"] != "code":
